...
#include "cudaMemoryPoolExecutor.H"
#include "error.H"
#include "cudaError.H"
...
__global__
void cudaMemSet(uint64_t nElementsInBytes, int dataSize, char *target, const char *source)
{
...
__global__
void cudaMemSetOne(uint64_t nElements, scalar *target)
{
...
void* Foam::cudaMemoryPoolExecutor::alloc(uint64_t size) const
{
    void* ptr;
...
void Foam::cudaMemoryPoolExecutor::clear(void* ptr) const
{
    if (ptr)
    {
...
void Foam::cudaMemoryPoolExecutor::memCopy(void* dst, const void* src, uint64_t size, memCopyKind kind) const
{
    label err = 0;
...
void Foam::cudaMemoryPoolExecutor::memSet(void* ptr, const size_t sizeInBytes, const void* value, size_t sizeOfValue) const
{
    ...
    Foam::cudaMemSet<<<numBlocks, NUM_THREADS_PER_BLOCK>>>
    (
...
void Foam::cudaMemoryPoolExecutor::memSetScalarOne(void* ptr, const size_t sizeInBytes) const
{
    const int numBlocks = SET_NUM_BLOCKS(sizeInBytes);
    Foam::cudaMemSetOne<<<numBlocks, NUM_THREADS_PER_BLOCK>>>
    (
...
void Foam::cudaMemoryPoolExecutor::memSet(void* ptr, const size_t sizeInBytes, const int value) const
{
    label err = CHECK_CUDA_ERROR
    (
        cudaMemset
