...
namespace Foam
{
    defineTypeNameAndDebug(aDICSmoother, 0);

    lduMatrix::smoother::addsymMatrixConstructorToTable<aDICSmoother>
        addaDICSmootherSymMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::aDICSmoother::aDICSmoother
(
...
) const
{
    ...
    for (label sweep=0; sweep<nSweeps; sweep++)
    {
        ...
        auto Lambda2 = [=](label face)
        {
            ...
            (
                rAtmpPtr[uPtr[face]],
               -rDPtr[uPtr[face]]*upperPtr[face]*rAPtr[lPtr[face]]
            );
        };
...
void Foam::aDICSmoother::scalarSmooth
(
    solveScalarField& psi,
